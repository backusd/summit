...
std::string Http::GetImpl(std::string_view url) const
{
    ...
    if (connect(sock, sockaddr_ipv4, sizeof(sockaddr_in)) != 0) 
    ...
    while (remaining > 0)
    {
        ...
        auto result = send(sock, data, remaining, 0); 
...
#ifdef SUMMIT_PLATFORM_WINDOWS
void Http::PrintAddrInfoDetails(addrinfo* addrInfo) const noexcept
{
